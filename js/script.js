...
(function ($, window) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateRating(data) {
      ...
      const weightedSum = Object.entries(data).reduce((sum, [rating, count]) => {
      ...
      document.querySelector('.review-count').textContent = `${totalReviews} reviews`;
    ...
    function updateStars(rating) {
      ...
      stars.forEach((star, index) => {
        if (index < fullStars) {
          ...
          star.style.background = `linear-gradient(90deg, #FDA769 ${decimal * 100}%, #ccc ${decimal * 100}%)`;
